...
namespace Dapper.Neat.Mapper
{
    ...
    public class StructureMap<TSource, TDestination> : IStructureMap<TSource>
    {
        ...
        private readonly List<IStructureMapItem<TSource, TDestination>> structureMapList = new List<IStructureMapItem<TSource, TDestination>>();
        ...
        public string GetColumns(string alias)
        {
            ...
            return String.Join(",", totalList.Select(x => String.Format("{0}.[{1}] as {2}", alias, x.DestinationName, x.SourceName)));
        }
        ...
        public DynamicParameters GetParameters(TSource Data, bool isInsert = true)
        {
            ...
            if (idStructureMap != null && idStructureMap.IsAssignable)
                parameters.Add(idStructureMap.DestinationName, idStructureMap.GetValueForDapperParameters(Data), null, isInsert && idStructureMap.AutoGenerated ? ParameterDirection.Output : ParameterDirection.Input);
            return parameters;
        }
        ...
        public void AutoMap()
        {
            ...
            var mInfo = typeof (StructureMap<TSource, TDestination>).GetMethods().FirstOrDefault(m => m.Name == "MapProperty" && m.GetParameters().Length == 2);
        ...
        {
            return (structureMapList.FirstOrDefault(sm => sm.SourceName == name)) ?? (idStructureMap.SourceName == name ? idStructureMap : null);
        }
        ...
        private void AddPropertyExpressionToMap(MethodInfo mapPropertyMethod, PropertyInfo sourcePropertyInfo, PropertyInfo destinationPropertyInfo)
        {
        ...
        private void BuildDeleteExpression()
        {
            if (idStructureMap == null)
                return;
            DeleteSqlTemplate = String.Format("Delete from [{0}] where [{1}] = @{1}", TableName, idStructureMap.DestinationName);
        }
        ...
        private void BuildInsertExpression()
        {
            ...
            string columnNames = String.Join(",", structureMapList.Select(x => String.Format("[{0}]", x.DestinationName)));
            ...
            if (idStructureMap != null && idStructureMap.IsAssignable && idStructureMap.AutoGenerated)
                insertSqlExpressionBuilder.AppendLine(String.Format("Select @{0} = SCOPE_IDData();", idStructureMap.DestinationName));
        ...
        private void BuildUpdateExpression()
        {
            ...
            string columnNames = String.Join(",", structureMapList.Select(x => String.Format("[{0}] = @{0}", x.DestinationName)));
